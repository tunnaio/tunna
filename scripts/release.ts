...
const [line, exact] = args.filter((a) => !a.startsWith("--")) as [
  Line | undefined,
  string | undefined,
];
...
if (line !== "server" && line !== "sdk") {
  console.error(
    "usage: bun scripts/release.ts <server|sdk> [version] [--dry-run] [--skip-ci-check]",
  );
  process.exit(2);
}

const text = async (cmd: ReturnType<typeof $>) =>
  (await cmd.quiet().text()).trim();
...
// A step is anything awaitable: a shell command or an async function.
const run = async (what: string, cmd: () => PromiseLike<unknown>) => {
...
if (head !== (await text($`git rev-parse origin/main`)))
  refuse("main is not level with origin/main; push or pull first");

async function ciIsGreen(sha: string): Promise<void> {
  if (skipCI) return console.log("  CI check skipped by flag");
  const out = await text(
    $`gh run list --workflow ci.yml --commit ${sha} --limit 1 --json status,conclusion`,
  ).catch(() => "");
  if (out === "")
    return refuse(
      "could not ask GitHub for the CI run (is gh installed and signed in?); --skip-ci-check overrides",
    );
  const [latest] = JSON.parse(out) as { status: string; conclusion: string }[];
  if (!latest)
    return refuse(
      `no CI run found for ${sha.slice(0, 7)}; wait for it, or --skip-ci-check`,
    );
  if (latest.status !== "completed")
    return refuse(`CI is still ${latest.status} on ${sha.slice(0, 7)}`);
  if (latest.conclusion !== "success")
    return refuse(`CI concluded ${latest.conclusion} on ${sha.slice(0, 7)}`);
...
async function next(l: Line): Promise<string> {
  const newest =
    (await text($`git tag --list ${PREFIX[l] + "*"} --sort=-v:refname`)).split(
      "\n",
    )[0] ?? "";
  if (newest === "")
    fail(`no ${PREFIX[l]}* tag yet; give the version explicitly`);
  const current = newest.slice(PREFIX[l].length);
  const m = /^(.*\.)(\d+)$/.exec(current);
  if (!m || !current.includes("-"))
    fail(`${current} is not a prerelease; give the next version explicitly`);
...
if (!VERSION.test(version))
  fail(
    `${JSON.stringify(version)} is not a version like 0.1.0 or 0.1.0-alpha.7`,
  );
const tag = PREFIX[line] + version;
if ((await text($`git tag --list ${tag}`)) !== "")
...
console.log(
  `  commit  ${head.slice(0, 7)}  ${await text($`git log -1 --format=%s`)}`,
);
console.log(`  tag     ${tag}`);
...
if (!dryRun) {
  ...
  if (typed?.trim() !== version)
    fail("the version typed did not match; nothing was changed");
}
console.log("");
...
if (line === "sdk") {
  ...
  await run(`set "version": "${version}" in ${pkgPath}`, async () => {
    ...
    if (bumped === pkg)
      fail(`${pkgPath} already says ${version}, or has no version field`);
    await Bun.write(pkgPath, bumped);
  });
  await run(
    `git commit -m "sdk/typescript ${version}" -- ${pkgPath}`,
    () => $`git commit --quiet -m ${"sdk/typescript " + version} -- ${pkgPath}`,
  );
...
await run(
  `git tag ${tag} ${dryRun && line === "sdk" ? "<the bump commit>" : tagged.slice(0, 7)}`,
  () => $`git tag ${tag} ${tagged}`,
);
...
if (line === "server") {
  ...
  console.log(
    "  then     redeploy, and move the pinned image in README.md and docs/deploy.md",
  );
} else {
  console.log(
    "  watch    gh run watch $(gh run list --workflow publish-typescript.yml --limit 1 --json databaseId --jq '.[0].databaseId')",
  );
