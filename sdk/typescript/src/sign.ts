...
function signedHeaderNames(req: SigningRequest): string[] {
...
function presignPairs(
  ...
  pairs.push(
    [PARAM_KEY, key.id],
    [PARAM_EXPIRES, String(expires)],
    [PARAM_HEADERS, signedHeaderNames(req).join(";")],
  );
  return pairs;
}
...
export function canonical(
  ...
  const query =
    mode === "header"
      ? encodeQuery(req.query ?? [])
      : encodeQuery(presignPairs(req, key, time));
...
export async function presignQuery(
  req: SigningRequest,
  key: Key,
  expires: number,
): Promise<string> {
  const sig = await signature(
    key.secret,
    canonical(req, key, expires, "presign"),
  );
